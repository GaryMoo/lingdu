...
class Spider(Spider):
        ...
        if '###' in did:
            ...
            if tag == 'wogg':
                ...
                for playInfos in playInfosList:
                    i += 1
                    playUrl += '#' + self.xpText(playInfos, "./text()").replace('\xa0', '') + '$' + self.xpText(playInfos, "./qyg11.js") + '---{}---{}'.format(name, i)
    ...
    def searchxy(self, key, tag, pg):
        ...
        count = 0
        for video in vList:
            name = self.xpText(video, './qyg7.js').strip()
            if name.count(key) > 1 or len(name) > len(key) + 20:
                name = ''.join(OrderedDict.fromkeys(name))
            vid = b64decode(self.xpText(video, './qyg8.js')).decode().replace('www.alipan.com', 'www.aliyundrive.com')
    ...
    def searchwogg(self, key, tag, pg):
        ...
        nextpage = True
        for video in vList:
            img = self.xpText(video, './qyg9.js')
            if not img.startswith('http'):
                img = self.regStr(img, '\((http.*?)\)')
            title = self.xpText(video, './qyg10.js').strip()
        ...
        if maxPageLiset != []:
            maxPage = self.xpText(maxPageLiset[-1], './qyg11.js')
